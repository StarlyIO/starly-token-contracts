...
describe("StarlyTokenStaking contract tests", () => {
    ...
    test("Tricky case: burn stake colletion", async () => {
        await shallPass(stake({signer: aliceAccount, amount: 100, addressMap}))
...
function setupStarlyTokenVault({signer, addressMap}) {
    return tx("../../../transactions/token/setup_account.cdc", [], signer, addressMap);
}

function transferStarlyTokens({to, from, amount, addressMap}) {
    return tx("../../../transactions/token/transfer.cdc", [amount, to], from, addressMap);
}

function setupStakingCollection({signer, addressMap}) {
    return tx("../../../transactions/staking/setup_account.cdc", [], signer, addressMap);
}

function enableStaking({signer, addressMap}) {
    return tx("../../../transactions/staking/admin/enable_staking.cdc", [], signer, addressMap);
}

function disableStaking({signer, addressMap}) {
    return tx("../../../transactions/staking/admin/disable_staking.cdc", [], signer, addressMap);
}

function enableUnstaking({signer, addressMap}) {
    return tx("../../../transactions/staking/admin/enable_unstaking.cdc", [], signer, addressMap);
}

function disableUnstaking({signer, addressMap}) {
    return tx("../../../transactions/staking/admin/disable_unstaking.cdc", [], signer, addressMap);
}

function setUnstakingFees({signer, unstakingFee, unstakingFlatFee, unstakingFeesNotAppliedAfterSeconds, addressMap}) {
    return tx("../../../transactions/staking/admin/set_unstaking_fees.cdc", [
        unstakingFee,
        unstakingFlatFee,
...
function createCustomStake({signer, address, amount, k, minStakingSeconds, addressMap}) {
    return tx("../../../transactions/staking/admin/custom_stake.cdc", [
        address,
        amount,
        k,
        minStakingSeconds,
...
function refund({signer, address, id, addressMap}) {
    return tx("../../../transactions/staking/admin/refund.cdc", [address, id], signer, addressMap);
}

function refundCustomK({signer, address, id, k, addressMap}) {
    return tx("../../../transactions/staking/admin/refund_custom_k.cdc", [address, id, k], signer, addressMap);
}

function setMinStakingSeconds({signer, seconds, addressMap}) {
    return tx("../../../transactions/staking/admin/set_min_staking_seconds.cdc", [seconds], signer, addressMap);
}

function setMinStakePrincipalAmount({signer, amount, addressMap}) {
    return tx("../../../transactions/staking/admin/set_min_stake_principal.cdc", [amount], signer, addressMap);
}

function setUnstakingDisabledUntilTime({signer, timestamp, addressMap}) {
    return tx("../../../transactions/staking/admin/set_unstaking_disabled_until_timestamp.cdc", [timestamp], signer, addressMap);
}

function stake({signer, amount, addressMap}) {
    return tx("../../../transactions/staking/stake.cdc", [amount], signer, addressMap);
}

function unstake({signer, id, addressMap}) {
    return tx("../../../transactions/staking/unstake.cdc", [id], signer, addressMap);
}

function unstakeAll({signer, addressMap}) {
    return tx("../../../transactions/staking/unstake_all.cdc", [], signer, addressMap);
}
...
function setStakingK({signer, k, addressMap}) {
    return tx("../../../transactions/staking/admin/set_k.cdc", [k], signer, addressMap);
}

function tamperPrincipal({signer, k, addressMap}) {
    return tx("../../../transactions/staking/tamper_principal.cdc", [], signer, addressMap);
}

function burnStake({signer, k, addressMap}) {
    return tx("../../../transactions/staking/burn_stake.cdc", [], signer, addressMap);
}

function burnStakeCollection({signer, k, addressMap}) {
    return tx("../../../transactions/staking/burn_stake_collection.cdc", [], signer, addressMap);
}
...
async function getStarlyTokenBalance(account, addressMap) {
    return await script("../../../scripts/token/read_balance.cdc", [account], addressMap)
}

async function getTotalPrincipalStaked(addressMap) {
    return await script("../../../scripts/staking/read_total_principal_staked.cdc", [], addressMap)
}
